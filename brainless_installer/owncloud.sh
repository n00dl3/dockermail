...
run_owncloud_server(){
    ...
    if [ $NGINX_PROXY -eq 1 ];then
      read -p "please, provide the virtualhost for accessing the owncloud container [owncloud.${DOMAIN_NAME}] :"
      local v_host=${REPLY:-"owncloud.$DOMAIN_NAME"}
      host_binding="-e VIRTUAL_HOST=${v_host}"
    else
      read -p  "Specify the interface to bind owncloud to [0.0.0.0] :"
      local OWNCLOUD_INTERFACE=${REPLY:-"0.0.0.0"}

      read -p "Specify port which owncloud will listen to for HTTP connections [7070] :"
      local OWNCLOUD_PORT=${REPLY:-"7070"}

      host_binding="-p ${OWNCLOUD_INTERFACE}:${OWNCLOUD_PORT}:80"
    fi

    read -p "please, provide an admin login for your owncloud installation [admin] :"
    local OWNCLOUD_ADMIN_USER=${REPLY:-"admin"}

    local OWNCLOUD_ADMIN_PASSWORD=""
    while [ ${#OWNCLOUD_ADMIN_PASSWORD} -lt 12 ];do
        read -p "please, provide an admin password for your owncloud installation :" OWNCLOUD_ADMIN_PASSWORD
    done
