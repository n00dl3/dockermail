run_mail_server(){
    ...
    #read port bindings
    read -p "Specify the interface to bind the mail server to [0.0.0.0] :"
    local DOVECOT_INTERFACE=${REPLY:-"0.0.0.0"}

    read -p "Specify the port for imap connections (TLS) [143] :"
    local IMAP_PORT=${REPLY:-"143"}

    read -p "Specify the port for imap connections (SSL) [993] :"
    local IMAPS_PORT=${REPLY:-"993"}

    read -p "Specify the port for incoming smtp connections [25] :"
    local SMTP_IN=${REPLY:-"25"}

    read -p "Specify the port for outgoing smtp connections [587] :"
