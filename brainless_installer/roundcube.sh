...
run_roundcube_server(){
    ...
    local NGINX_PROXY=$3
    stop_container roundcube
    if [ $? -gt 0 ];then
      return 2
    fi
    #configure run variables
    if [ $NGINX_PROXY -eq 1 ];then
        read -r -p "please, provide the virtualhost for accessing the roundcube container [roundcube.${DOMAIN_NAME}] :"
        local vhost=${REPLY:-"roundcube.$DOMAIN_NAME"}
        local ROUNDCUBE_VHOST="-e VIRTUAL_HOST=$vhost"
    else
      read -r -p "Specify the interface to bind roundcube to [0.0.0.0] :"
      local ROUNDCUBE_INTERFACE=${REPLY:-"0.0.0.0"}

      read -r -p "Specify port which roundcube will listen to for HTTP connections [8080] :"
